...
namespace Player
{
    public class PlayerStateMachine : MonoBehaviour
    {
        ...
        [Range(-5, 5)][SerializeField] float _targetVaultOffset = 0;
        ...
        {
            _currentAnimationBlendVector = Vector2.SmoothDamp(_currentAnimationBlendVector, input, ref _animationVelocity, _animationSmoothTime);
            Vector3 _climbDirection = Vector3.zero;
            _climbDirection.x = 0f;
            _climbDirection.z = 0f;
            _climbDirection.y = _currentAnimationBlendVector.y;
            _characterController.Move(_climbDirection * _speed * Time.deltaTime);
        }
        ...
        private void GroundFrontCheck()
        {
            ...
            if (Physics.Raycast(_groundFrontTargetOrigin, -transform.up, out groundFront, _groundFrontCheckDistance, _groundCastMask))
            {
                ...
                _targetVaultPosition.x = _groundFrontTargetOrigin.x;
                _targetVaultPosition.z = groundFront.point.z;
                _targetVaultPosition.y = groundFront.point.y + _targetVaultOffset;
            }
        }
        ...
        private void SlopeCheck()
        {
            ...
            if (_gotNeFoot && _groundFrontDistance < _initialGroundDistance && !_gotLowWall)
            {
